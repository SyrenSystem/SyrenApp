            ...
            Expanded(
              child: distanceItems.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) => SpeakerVolumeRow(
                        key: ValueKey(distanceItems[index].id),
                        item: distanceItems[index],
                      ),
                    ),
            ),
          ],
...
class _EmptyVolumeView extends StatelessWidget {
  const _EmptyVolumeView();
...
class SpeakerVolumeRow extends ConsumerWidget {
  const SpeakerVolumeRow({required this.item, super.key});

  final DistanceItem item;
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final coordinator = ref.read(volumeCommitCoordinatorProvider);
    return Container(
        ...
        children: [
          ...
          SliderTheme(
            ...
            child: Slider(
              ...
              max: 100,
              divisions: 100,
              onChanged: (value) => coordinator.update(item.id, value),
              onChangeEnd: (value) => unawaited(coordinator.flush(item.id)),
            ),
          ),
