...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='syren-rtp-smoke-') as temporary:
        ...
        forward_port = ingress.prepare()
        configuration = (DIRECTORY / 'templates/receiver.conf.in').read_text()
        configuration = configuration.replace('@PORT@', str(forward_port)).replace('@DEVICE@', 'hw:sndrpihifiberry,0')
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as proxy:
                ...
                for attempt in range(1):
                    ...
                    assert ingress.filter.accepted >= 100
                    receiver_node = next(item for item in objects if item.get('info', {}).get('props', {}).get('node.name') == 'syren_rtp_receive')
                    assert receiver_node['info']['props'].get('rtp.receiving') in (True, 'true')
                    sockets = subprocess.check_output(['ss', '-u', '-a', '-n'], text=True)
                    assert f'127.0.0.1:{forward_port}' in sockets, sockets
                    assert f'0.0.0.0:{forward_port}' not in sockets, sockets
                ...
                print(json.dumps({'packet_counts': packet_counts, 'pcm_frames_per_packet': 120,
                                  ...
                                  'initial_software_volume': 10, 'ingress': ingress.filter.status()}))
        except Exception:
